...
public class SqlServerDatabaseMeta extends BaseDatabaseMeta {
	...
	public String getUpsertStatement(List<String> column, String table, Map<String, List<String>> updateKey) {
		...
		if (CollectionUtils.isEmpty(updateColumns)) {
			return "MERGE INTO " + quoteTable(table) + " T1 USING " + "(" + makeValues(column) + ") T2 ON ("
					+ updateKeySql(updateKey) + ") WHEN NOT MATCHED THEN " + "INSERT (" + quoteColumns(column)
					+ ") VALUES (" + quoteColumns(column, "T2") + ");";
		} else {
			return "MERGE INTO " + quoteTable(table) + " T1 USING " + "(" + makeValues(column) + ") T2 ON ("
					...
					+ "INSERT (" + quoteColumns(column) + ") VALUES (" + quoteColumns(column, "T2") + ");";
		}
	}
	...
	private String getSqlServerUpdateSql(List<String> column, Map<String, List<String>> updateKey, String leftTable,
			String rightTable) {
